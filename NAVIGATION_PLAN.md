...
## 省空间：底栏浮起 + 滚动隐藏 + 沉浸模式（0.8.0）

方屏机器（Titan Lite2）上竖向空间很紧，底栏 78dp + 状态栏 24dp 占掉可视高度的六分之一左右。

### 关键区分：压缩视口 vs 浮在上面

原来是给 WebView 设 `bottomMargin = dp(78) + inset` —— **视口直接变矮**，X 以为屏幕就这么高，少渲染那么多内容。

改成 margin 归零、底栏浮在正文上之后，视口拿回完整高度，**多出来的是实打实多渲染的内容**。为了不让文档末尾被压住，通过 `setChrome()` 给页面下发 `body { padding-bottom }` —— 时间线是无限的，那段 padding 永远滚不到，等于纯赚；短页面也只是末尾多一段可滚动空白。

**这是 `setChrome()` 接口第一次真正派上用场**（P1 时留了接口但刻意没下发数值，因为当时底栏是占位的，发 0 会改掉 `j`/`k` 落位）。

### 滚动隐藏（0.8.1 修正）

**最初只用原生 `WebView.setOnScrollChangeListener`，在真机上完全不工作。**

原因：**X 的时间线是内部容器在滚，不是整页滚动。** 原生滚动监听只能感知 WebView 自身的滚动位置，对容器内部的滚动收不到任何事件。

我当时的判断依据是「`window.scrollBy`（空格翻页、`j`/`k`）能正常工作，说明是整页滚动」—— 这个推理是错的：`window.scrollBy` 在容器滚动的页面上也不会报错，只是没效果或效果有限，光看「没报错」推不出滚动模型。redroid 上的登录页恰好是整页滚动，于是本地测试通过，真机才暴露。

修法：在注入脚本里用**捕获阶段**监听 `document` 的 scroll 事件（`addEventListener("scroll", fn, true)`）—— 冒泡阶段收不到 scroll，但捕获阶段能抓到任意元素的滚动。位置取 `event.target.scrollTop`，整页滚动时回退到 `window.scrollY`。原生监听保留，两条路并存。

一个容易踩的细节：**换滚动容器时必须重置基准**，否则两个容器的坐标系会算出垃圾差值（比如从 scrollTop=5000 的时间线切到 scrollTop=0 的详情页，会误判成向上滚了 5000）。

验证方式：`testing/test-timeline.html` 改成了内部容器滚动的结构，专门复现这个 case。

规则：`scrollY <= 8dp` 显示（刚打开页面能看见）→ 向下滚 3dp 以上隐藏 → 向上滚 10dp 以上显示。向上的阈值故意比向下大，避免手抖来回抽搐。抽屉、快捷键页、发帖对话框打开时不响应滚动。

### 0.8.2：再修一次 + 加诊断

0.8.1 在真机上仍然完全不隐藏。复盘发现 0.8.1 的实现里有个自己引入的 bug：

```js
if (target !== lastScrollTarget) { lastScrollTarget = target; lastScrollPos = pos; return; }
```

用**单个变量**存「上一个滚动目标」。如果页面上有多个元素交替派发 scroll（容器一个、document 一个），那么每一次事件都会命中「目标变了」这个分支并 return，**永远算不出差值**，底栏自然一动不动。

改成 `WeakMap` 按元素分别记位置，谁滚就跟谁比。同时把向上的判定改成累计位移（`upAccum > 24`），单次事件的位移往往只有几 px，用单次阈值会漏。

**更重要的是加了诊断**：抽屉底部一行实时显示 `版本 · 滚动事件数 · 滚动目标 · 位置 · 底栏状态`。前两次都是靠猜改的，改完还得等真机反馈一轮，效率太低。有了这行，真机上一眼就能定位是「事件没触发」「目标不对」还是「原生侧被门禁挡住」。

顺带把版本号也显示出来了 —— 之前根本没法确认用户装的是哪个构建。

### 副作用：底栏收起时抽屉入口也没了

`我` 按钮在底栏上，底栏一收起就点不到，而左边缘右滑又被系统返回手势吃掉。目前只能先向上滚把底栏唤回来再点。不算阻塞（向上滚很轻），但记在这里。

### 两个模式的语义不同

- **自动隐藏**：WebView margin = 0，底栏浮在上面，页面补 padding
- **常驻**：恢复 `bottomMargin = dp(78)`，页面不补 padding（位置已经让出来了）

不是简单地「藏起来/显示」，布局策略整个不同。

### 顺带修的

底栏浮到正文上之后，原来 `0xf2` 的半透明底色会把下面的文字透出来，很脏。改成完全不透明，并用 `mix()` 和页面底色拉开一档（浅色压暗 6%，深色提亮 10%），这样在纯黑和纯白页面上都看得出边界。

### 沉浸模式

`WindowInsetsController.hide(statusBars())` + `BEHAVIOR_SHOW_TRANSIENT_BARS_BY_SWIPE`（API 30+），低版本走 `SYSTEM_UI_FLAG_FULLSCREEN | IMMERSIVE`。默认关，抽屉里开。两个全开约多出 102dp。
