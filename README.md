...
- **底部悬浮导航**：`首页` `搜索` `Grok` `通知` `私信` `我`，带选中态和未读角标。默认**向下滚动就收起**，向上滚或回到顶部再出现 —— 方屏上这条底栏占 78dp，收起后是实打实多渲染出来的内容，不是把内容压在下面
- **沉浸模式**（抽屉里开）：连系统状态栏一起隐藏，再多 24dp，从顶部下滑可临时唤出
...
方屏机器上竖向空间很紧，底栏和状态栏加起来能占掉可视高度的六分之一。抽屉里两个开关：

| 开关 | 说明 |
| --- | --- |
| `底栏自动隐藏` / `底栏常驻` | 自动隐藏时底栏浮在正文上，**视口拿回完整高度**，只在文档末尾补一段 `padding-bottom`（时间线无限，那段永远滚不到）。常驻时恢复成给底栏留位置 |
| `沉浸模式` | 隐藏系统状态栏，再多 24dp |

两个全开大约多出 **102dp** 的内容高度。
